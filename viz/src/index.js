...
fetch('/output.json').then((v) => v.json()).then((data) => {
	...
	const outputFiles = data.bundleFileStats
		.sort((a, b) => {
			return b[2].highestBundle + b[2].pctOverlap - (a[2].highestBundle + a[2].pctOverlap);
		})
		.map((d) => {
			d[3] = nest()
	...
	const networkLinks = data.graph.links.filter((d) => relatedNodes.indexOf(d.target) !== -1);
	...
	const networkNodes = data.graph.nodes.filter((d) => relatedNodes.indexOf(d.id) !== -1);
	...
	render(
		<Router>
			<div>
				<Route exact path="/" component={(props) => <AppState {...props} passedData={passedData} />} />
				<Route path="/:id" component={(props) => <AppState {...props} passedData={passedData} />} />
			</div>
		</Router>,
		target
	);
